// Dot Matrix Animation - Animated background with red dot matrix pattern for master pages
// This script creates an animated dot matrix background with red dots

document.addEventListener('DOMContentLoaded', function() {
    // Create canvas element for the animation
    const canvas = document.createElement('canvas');
    canvas.id = 'masterFlowCanvas';
    ...
    const dotMatrixAnimation = new MasterDotMatrixAnimation(canvas);
    dotMatrixAnimation.start();
});

class MasterDotMatrixAnimation {
    constructor(canvas) {
        this.canvas = canvas;
        ...
        this.baseHue = 220; // Base color (blue/azure)
